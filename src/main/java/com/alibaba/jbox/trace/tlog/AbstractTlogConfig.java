...
public abstract class AbstractTlogConfig implements Serializable {
    ...
    private int minPoolSize = TlogConstants.MIN_THREAD_POOL_SIZE;

    private int maxPoolSize = TlogConstants.MAX_THREAD_POOL_SIZE;

    private int runnableQSize = TlogConstants.DEFAULT_RUNNABLE_Q_SIZE;

    private int maxHistory = TlogConstants.DEFAULT_MAX_HISTORY;

    private String placeHolder = TlogConstants.PLACEHOLDER;
    ...
    private String charset = TlogConstants.UTF_8;
    ...
    public void setResource(Resource resource) throws IOException {
        ...
        Preconditions.checkArgument(StringUtils.equals(fileType, TlogConstants.XML_FILE_SUFFIX), "config file is not xml format.");
//    todo    ELXmlResolver.resolveConfigFile(resource.getInputStream(), methodELMap, fileName);
    }
    ...
    String getUniqueLoggerName() {
        if (Strings.isNullOrEmpty(this.uniqueLoggerName)) {
            ...
            int suffixIndex = loggerName.lastIndexOf(TlogConstants.LOG_SUFFIX);
            if (suffixIndex != -1) {
